...
class CheckoutView(CartMixin, View):
    def get(self, request):
        ...
        if cart.total_items == 0:
            if request.headers.get('HX-Request'):
                return TemplateResponse(request, 'orders/empty_card.html', {'message': 'Your cart is empty.'})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            for item in cart.items.select_related('product', 'product_size__size').order_by('_added_at'):
